...
AFRAME.registerComponent('fire', {
  ...
  tick: function (time, delta) {
    ...
    var invModelMatrix = this.fire.material.uniforms.invModelMatrix.value;
    //console.log(invModelMatrix);

    mesh.updateMatrix();
    invModelMatrix.getInverse( this.fire.matrix );

    this.fire.material.uniforms.time.value = time / 1000; //always needed for flame flicker

    this.fire.material.uniforms.invModelMatrix.value = invModelMatrix;
    
    //this.material.uniforms.scale.value.set(3, 3, 3);

    mesh.material.uniforms.scale.value = this.el.getAttribute('scale');
    //this.fire.material.uniforms.scale.value = this.fire.scale;

    //this.el.setAttribute('scale', this.fire.scale);

    //console.log(this.el.getAttribute('scale'));

    /*var mesh = el.getObject3D('fire-mesh');
    console.log(mesh);
    if (mesh) {
